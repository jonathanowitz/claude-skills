...
**Scope note:** The universal rules here (behavioral-RED discipline, locator stability, timing patterns) apply to any Playwright suite. The concrete paths and examples (`web/*.html`, `example-context/architecture/app-behavior-map.md`, vanilla-JS init gotchas like `clearStaleAppCache`) are drawn from example-app's stack as a worked example — resolve the equivalent behavior-map location and markup/component source for whichever repo is actually under test.

## CI Pipeline Topology (example-app) — read this before debugging a red run

The whole test pipeline, so you don't reconstruct it from `api-integration.yml` under pressure. All jobs run against **one shared, persistent Supabase e2e branch** (project ref `<E2E_PROJECT_REF>`), not an ephemeral DB.

- **Trigger:** the suite fires on **`deployment_status`** — i.e. AFTER Vercel finishes deploying the PR's preview, not on push. So a red "api-integration" run is always testing a *specific deployed preview*; check the preview sha matches HEAD before trusting a failure.
- **`api-tests` job** runs first: `pnpm run refresh-e2e-data` (re-seeds the branch) → `vitest run --config vitest.integration.config.js` with `TEST_BASE_URL` = the preview. This is the API/DB-contract layer.
- **`smoke-tests` job** is **chained behind it (`needs: api-tests`)** — it only runs if api-tests is green. It re-seeds again (own pristine baseline) and then runs THREE separate sequential Playwright steps: **smoke** (`--project=smoke`), **journey** (`--project=journey`, chromium), **webkit-journey** (`--project=webkit-journey`, real WebKit = the Capacitor iOS runtime, the FS tap-regression specs). They are separate `run:` steps → serial, never concurrent with each other.
- **Parallelism:** no `fullyParallel` and no `workers` override in `playwright.config.js`, so on the 2-core CI runner it's effectively **1 worker → journey spec files run serially in alphabetical order**. Specs rely on this (a `beforeAll` publish is the live set when that file's own test runs).
- **Seed authority:** **`e2e/lib/refresh-data.js`** (`refreshE2EData()` orchestrator) is the durable seed for the persistent branch, run every job via `pnpm run refresh-e2e-data`. `supabase/migrations/.../seed.sql` runs **only at branch creation** — it is NOT the per-run seed. A fixture that must exist every run goes in refresh-data.js, not seed.sql.
- **Alphabetical ordering is load-bearing and dangerous:** an earlier-sorting spec can mutate account-wide state a later spec depends on (e.g. `1206-*` wipes `E2E_PRODUCER`'s memberships account-wide before `home-*`/`s-fs*` run). See the account-wide-wipe rules under Seed Data below, and the auth-gate rule directly under this section.
- **`scripts/check-journey-coverage.mjs` matches its detector regexes over the WHOLE spec file, comments included.** It classifies a spec's public-vs-authed "floor" with plain regexes like `/signInViaUI\s*\(/` — and the match is not scoped to code, so a docstring that mentions the call in prose trips it. Writing `signInViaUI (` inside a comment (e.g. "…NO signInViaUI (signing in would…)") flips a genuinely-public spec to read as authed, and the gate then rejects it with a confusing "needs a public-surface journey" message. Never write `signInViaUI` immediately followed by `(` in prose — phrase it "NO sign-in call" or break it with an em-dash. The same whole-file substring/regex matching applies to the authed-floor legs (`toBeVisible` / `goto` / `click`), so keep those tokens out of prose too. (Evidence: 2026-08-04 #1226/PR #1286 — a docstring flipped a public spec to FAIL the public floor; cost one debug cycle.)

## When you add an authorization gate to a shared handler, every fixture that drives it breaks — patch them in the SAME change

Adding a server-side authority check (`requireX`, an RLS tightening, a new RPC gate) to a handler silently breaks **every test fixture that calls that handler without establishing the new authority** — journey `beforeAll` seeds, smoke setups, api-integration fixtures. They start returning 403 (then cascading 500s downstream), and because it's fixture setup, the failure looks like an unrelated red spec, not "I changed the gate." Before merging a gate change: **grep the whole `e2e/` + `tests/` tree for callers of the gated route** (`grep -rn 'gated-route-name' e2e tests`) and give each the authority it now needs, idempotently, in the SAME PR. Prefer a **shared, order-independent helper** the fixtures call in their own `beforeAll` (self-heals even after another spec's account-wide wipe) over a one-time seed (which a later-sorting spec can undo mid-run).
  (Evidence: 2026-07-31 #1248/PR #1250 — `requireCompetitionAdmin` was wired into 5 producer handlers; ~60 api-integration failures + a cascade of red journey/smoke specs, all because fixtures drove those handlers as `E2E_PRODUCER` with no/stale ownership. Root cause took a full session to trace because the CI topology + seed authority + alphabetical account-wide-wipe ordering had to be reconstructed from scratch — this whole section exists so that's a doc lookup next time. Fix: `e2e/lib/ensure-producer-admin.js` — idempotent `ensureE2EProducerAdmins(competitionId)` called first in each affected `beforeAll`.)
...
0. **Check behavior map preconditions** — Read the entry in the repo's behavior map for the feature you're testing (resolve via the repo's `context_repo` declaration if it has one — see `references/product-json.md`; example-app → `example-context/architecture/app-behavior-map.md`). Each entry has two layers:
...
1. **Verify element selectors against the actual markup/component source** — grep the repo's own HTML/component files for the actual element IDs (example-app example: `web/*.html`). Never trust JS/component code that references them — `getElementById` calls (or the framework's element-lookup equivalent) for removed IDs silently return `null`/nothing. The code and markup can drift apart without errors.
...
- **Make a fixture durable by SEEDING it in `refreshE2EData()`, never by EXEMPTING it from the wipe.** The refresh's model is wipe-to-empty then rebuild from clean: `clearSpinePublishState` (and the per-table clears) delete the world, then the `seedXxx()` functions repopulate it. Any fixture that must exist every run — a spec fixture OR a `/walk` fixture — belongs in that seed path: a `seedXxxFixture(supabase)` added to `refreshE2EData` *after* the clear, so every run rebuilds it from scratch with canonical values. Do **NOT** make a fixture survive by adding its ids to a wipe exemption (`.neq('competition_id', …)` / `.not('… in', …)`). An exemption preserves whatever state the row happened to have when it was last written: it drifts silently, never re-derives its dates/relations, and accumulates run over run — the exact way the shared DB rots into stale, out-of-date data. The lone `WALK_RESULTS_COMP_ID` exemption in `refresh-data.js` is the OLDER pattern — treat it as legacy, do not copy it; the seed-from-clean rule supersedes it. (This is the same authority as the CI-topology note above: "a fixture that must exist every run goes in refresh-data.js, not seed.sql" — this bullet adds *how*: seed it, don't exempt it.)
  (Evidence: 2026-08-04 #1226 — the fan competitions-browse walk fixture (five published comps) vanished the first time a Preview Tests run wiped the branch. It was first "fixed" by exempting its comp ids from `clearSpinePublishState`, mirroring `WALK_RESULTS_COMP_ID`. USER rejected the whole exemption approach — "that's how the db turns to trash… start with a fresh db and just SEED it properly" — and it was reworked into `seedCompetitionsBrowseFixture()` called from `refreshE2EData`, with the standalone walk script importing that one seeder. **Second correction of the same seeding-method mistake**, which is why this rule is now written down.)

- **One dedicated competition per publishing spec — never share a comp across specs that publish/seed their own rows.** The e2e branch is one shared, mutable DB and journey specs run serially, so a comp shared across specs accumulates every spec's rows. Publish is **comp-scoped supersede-missing + latest-publish-wins**: a later spec's `beforeAll` rewrites an earlier spec's live set, and multiple same-named rows coexist within the comp — so a bare name-text locator (`filter({ hasText: 'Blue' })`, `getByRole('button', { name: /orange/i })`) resolves to 2+ elements and the assertion passes/fails for the wrong reason. **The fix is per-spec comp isolation, mirroring the api-integration convention `tests/api-integration/s1-spine-fixtures.js:44-77` ("ONE dedicated competition row PER DESCRIBE block", with a reserved UUID block).** A spec that only *reads* the shared spine roster can keep the shared comp; any spec that **publishes or direct-seeds** its own rows needs its own competition id. Defensive per-assertion scoping (anchoring `^team`, adding a per-spec `Hall` filter) is a **stopgap, not the fix** — it defuses one assertion while leaving the next one a landmine that only reddens on the slow shared-branch Preview Tests run. If per-spec comps are too big a change right now, the minimum is an `afterAll` that deletes the spec's seeded rows so they don't leak forward.
  (Evidence: 2026-07-07 #1020/#1024 — 11 journey/smoke specs + `e2e/lib/refresh-data.js` all shared `ACMES_COMP_ID` (`…900009`). fs1/fs3/fs5 reds were "two Blue cards" (`Hall A` from fs1, `Hall FS3` from fs3, `Hall FS5` from fs5) inside one comp. #1029 fixed the *assertions* (option a — hall-scoping + `^orange`); the structural fix — per-spec comps / afterAll cleanup — was deferred to a standing issue. The api-integration layer already solved this exact problem per-describe; the journey layer never adopted it.)
...
- **Specs must never seed, wipe, or sign in as a walk-dedicated account — the account-wide wipe is namespace-blind.** There is now a dedicated cast of walk-only roles (`E2E_WALK_*` / `e2e-walk-*@test.example.com`, authority: `e2e/test-config.js`), reserved for the `/walk` skill (`~/Projects/claude-config/commands/walk.md`). A journey spec's teardown clears a signed-in actor's memberships/state **by category, account-wide** — that's correct for journey isolation, but it doesn't know or care whether a walk fixture happens to be sitting on that same account. If a spec's setup or teardown ever touches an `e2e-walk-*` account, it will destroy a walk mid-review the next time Preview Tests runs. Fix on the spec side: never hardcode or reuse a walk-role account as a spec fixture, and if a helper resolves accounts by category (all coach accounts, all operator accounts), exclude the walk roles explicitly. (Evidence: 2026-07-28 #1224 — a walk fixture seeded onto the *shared journey* coach account, `e2e-coach@test.example.com`, was wiped by the account-wide CI teardown twice in one day; this is what drove the dedicated walk-account split, superseding the 2026-07-17 "reuse the journey cast" decision below the line.)
- **A walk-seed still must not copy a journey spec's fixture IDs/emails — but the defense is the walk-dedicated ACCOUNT above, not a separate ID namespace.** The mechanism is unchanged and still worth knowing: the `deployment_status`-triggered "Preview Tests" workflow runs the REAL journey specs (with their `beforeAll` seed + `afterAll` teardown) against the SAME shared e2e Supabase DB a manual walk-seed targets, on every preview deploy/redeploy, and a journey's `afterAll` deletes by its own declared fixture IDs — so a walk-seed that mirrors those IDs gets torn down mid-review. Diagnostic signature: not just the pending row but the WHOLE fixture gone (memberships + delegations deleted, `can_administer_*` returns false) = a journey teardown, not a partial user action. **What changed (2026-07-28): a disjoint ID namespace does not actually protect a walk, because the account-wide wipe is namespace-BLIND — it deletes every membership row on the signed-in account whatever ID block it lives in.** So build the fixture on a walk-dedicated `E2E_WALK_*` role (rule above) and give it an ID segment of its own for legibility; the account is what makes it safe, the segment just makes it readable. **`scripts/walk-namespace.mjs` (`walkUuid` / `walkEmail` / `assertNotJourneyNamespace`, the `@walk.example.com` domain) is DELETED — do not reintroduce it.** It encoded the superseded theory, and by the end it would have rejected all 22 members of the sanctioned walk cast, since every one of them is a `00000000-e2e0-…` / `@test.example.com` value by design. **The committed reference example is `scripts/seed-walk-operator.mjs`** — role-named not ticket-named, idempotent, and self-verifying at the layer the walk uses. Secondary rule, unchanged: don't redeploy after seeding a walk unless you re-seed afterward (a new deploy → `deployment_status` → Preview Tests → teardown).
  (Evidence: 2026-07-12 #1060/PR #1062 M3b walk — inbox empty mid-review; the walk-seed had copied `m3b-join-requests-inbox.spec.js`'s `00000000-e2e0-…` IDs + `m3b-*@test.example.com` emails, and the 14:48 Preview Tests run's `afterAll` deleted them. Filed + fixed as #1067 — whose namespace fix was retired by #1134 after 2026-07-28 #1224 showed the wipe to be namespace-blind and the shared *account* to be the real hazard.)

- **A walk fixture that changes a shared cast account's SCOPE SHAPE breaks any spec asserting that shape — and the spec cannot defend itself.** This is the inverse of the namespace rule above: there, the journey's teardown deleted the walk's rows; here, the walk's *extra* rows survive the journey's teardown and poison it. A journey spec's `wipeState` deletes by the IDs **it** created, so anything a walk seed granted the same account beyond those IDs is invisible to it and persists into the next CI run. Scope shape is what landing ladders, pickers and "sole X" guards branch on, so one extra membership silently reroutes the app and the spec fails on an assertion that has nothing to do with the seed. **The fix is on the spec side: clear the account's scope by CATEGORY (every team membership on that account), not by enumerated ID** — a teardown that knows less than setup wrote is the defect. Secondary: after signing off a walk, reset the fixture to the CI-expected shape rather than leaving it armed.
  (Evidence: 2026-07-27 #1130 — the walk seed gave the shared cast coach `e2e-coach@test.example.com` a second team so the multi-team picker leg could be walked. That pushed `resolveLandingRoute` from rung 3 (straight to the team) to rung 4 (the picker), so `1130-operator-team.spec.js`'s single-team test never found its `heading 'Comets'`. It stayed red for three days and was only caught by `gh pr checks` at merge time. `wipeState` cleared `PROGRAM_ID` and `TEAM_ID`; `TEAM2_ID` existed only in the walk script. A second instance of the same shape shipped in the same PR: the `1130-*.db.test.js` suite's `afterAll` tore down one of its four accounts and the rest collided with `idx_memberships_unique_active_team` on the next run. Both folded into #1224.)

- **Don't brand-prefix e2e fixture names — a "realistic" comp/team/gym name that borrows the "Example Project" brand string collides with brand-text locators on shared public surfaces.** Public pages (the fan home, marketing shell) assert on `getByText('Example Project')` for the brand header; a globally-reseeded fixture named e.g. `Example Project Coastal Classic` surfaces on that same public home and pushes the locator to 2+ matches → unrelated smoke specs go strict-mode-red at merge-gate time, far from the spec that introduced the name. Pick fixture names that read as realistic gym/comp names without echoing the product's own brand text.
  (Evidence: 2026-08-03 #1269 — `seed-fan-walk.js`'s durability fixture was named `Example Project Coastal Classic`; it leaked onto the public no-login home and collided with `FanShell`'s brand text, redding `home-to-schedule.spec.js` (strict-mode 2-element violation) only when Preview Tests ran the full shared-branch suite. Fixed by rename to `Coastal Classic Championship`, not by loosening the smoke spec's locator.)
