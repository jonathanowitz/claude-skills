...
|------------|-------------|
| Element selector / DOM ID | `grep` the actual HTML files (`web/*.html`) — JS references can be stale |
...
2. **Run the smoke test yourself before asking USER** — for any API endpoint change, use the Vercel preview URL + e2e credentials to verify the live response rather than handing USER a curl command. The pattern:
   a. `gh pr view <N> --json statusCheckRollup` → extract the Vercel preview URL from PR comments (`gh api repos/USER/example-app/issues/<N>/comments --jq '.[].body' | grep -o 'https://example-app[^)]*'`)
   b. Sign in via Supabase auth using creds from `$HOME/Projects/example-app/.env.test` — load with `source .env.test` (never inline secrets), POST to `$TEST_SUPABASE_URL/auth/v1/token?grant_type=password`
...
1. **Verify every selector against the HTML** — `grep` the actual `web/*.html` files for each element ID used in the test. Don't trust JS code that references IDs.
