...
3. **Brief** — Look for the shaped brief in the current repo's briefs location (example-app resolves this via its `context_repo`, `example-context/briefs/`) that matches the feature. Use the Verification section and Behavior Map Entries.
4. **Behavior map** — Read the current repo's behavior map (example-app's is `example-context/architecture/app-behavior-map.md`) for canonical behaviors.
...
- **Assertion-COMMENT mismatch (BLOCKER)** — Read each assertion against the prose introducing it: its own `it(...)` title, the comment block directly above it, and its `describe(...)` header. Where they disagree, the file states two different contracts and at most one is the intended behavior — flag it and name which sources back each side, rather than assuming the assertion wins. Prose and assertions are usually read as separate layers, so this survives every reviewer who evaluates assertions only against the feature. (Evidence: 2026-07-27 #1224 — an assertion expected an unheld URL scope to fall through to a default while its own describe-block header, four lines above, said it resolves to null; three Claude agents and two Gemini calls all missed it, and it surfaced only when an implementing agent could not satisfy both.)
...
5. **Split every finding into HAZARD and REMEDY, and verify them separately.** A finding bundles two independent claims — "here is a failure" and "here is the fix" — and they can have different truth values. A correct hazard lends unearned credibility to whatever fix rides along with it: you verify the failure, feel the finding check out, and adopt the remedy untested. Trace the proposed fix through the *same concrete fixture* that demonstrates the hazard; if it does not prevent that exact case, say so and solve it yourself. Confirming the hazard is not confirming the finding. This applies to the Stage 2 Claude agents' findings too, not just Gemini's. (Evidence: 2026-07-27 #1224 — a correctly-identified redirect hazard came with a prescribed fix whose own condition excluded the case it was prescribed for.)
...
3. Categorize every test:
   ...
   - **Vacuous fixture — assertion subject is a fixture, not a return value (BLOCKER)** — Behavioral RED does NOT prove non-vacuity when the thing asserted is a *precondition you set up* rather than a *value the code produced*. For every test asserting a throw, rejection, error status, or impossibility (`.toThrow()`, `.rejects.*`, `expect(status).toBe(4xx)`, "does NOT allow", "blocks the duplicate"), name the exact fixture field or row that CREATES the asserted condition — the duplicate row that trips the unique index, the null `auth.uid()`, the missing FK target, the over-limit value. If you cannot point to the fixture element that makes the guard fire, the test is vacuous: it goes green whether or not the guard exists, because the precondition it claims to exercise was never constructed. This is distinct from a stub-pass (that's about the no-op impl); a vacuous fixture fools the REAL implementation too. Fix by adding the fixture element that triggers the condition, then confirm the throw actually fires. (Evidence: 2026-07-09, two decidr sessions — tests asserting a rejection whose triggering fixture row was never inserted, so they went green against a correct impl for the wrong reason. Behavioral-RED cleared them because the assertion's subject was the fixture, not a return value.)
